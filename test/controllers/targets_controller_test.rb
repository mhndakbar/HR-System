...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference("Target.count") do
      post targets_url, params: { target: { description: @target.description, finish_date: @target.finish_date, start_date: @target.start_date, status: @target.status, team: @target.team, title: @target.title } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { description: @target.description, finish_date: @target.finish_date, start_date: @target.start_date, status: @target.status, team: @target.team, title: @target.title } }
